...
type CreateCompletionInput = Pick<
  ...
  'event_id' | 'team_id' | 'board_tile_id' | 'completed_by_user_id' | 'proof_url'
>
...
export async function listCompletionInspectionRows(eventId: string) {
  return getDb()
    ...
    .innerJoin('users', 'users.id', 'team_tile_completions.completed_by_user_id')
    .innerJoin('board_tiles', 'board_tiles.id', 'team_tile_completions.board_tile_id')
    .select([
